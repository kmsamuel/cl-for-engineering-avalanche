...
from avalanche.models import MultiTaskModule, avalanche_forward, avalanche_forward_base
...
class LearningWithoutForgetting(RegularizationMethod):
    ...
    def _lwf_penalty(self, out, x, curr_model):
        ...
        if self.prev_model is None:
            ...
            if isinstance(self.prev_model, MultiTaskModule):
                ...
                with torch.no_grad():
                    y_prev = avalanche_forward_base(self.prev_model, x, None)
                ...
                y_curr = avalanche_forward_base(curr_model, x, None)
    ...
    def update(self, experience, model):
        ...
        task_ids = experience.dataset.targets.uniques
        ...
        for task_id in task_ids:
            ...
            task_indices = [i for i, t in enumerate(experience.dataset.targets) if t == task_id]
            task_data_targets = [experience.dataset.targets[i] for i in task_indices]
            pc = set(torch.unique(torch.tensor(task_data_targets)).tolist())
...
class LearningWithoutForgettingRAADL(RegularizationMethod):
    """Learning Without Forgetting.
    ...
    def _lwf_penalty(self, out, x, fc, curr_model):
        """
        Compute weighted distillation loss.
        """
        if self.prev_model is None:
            ...
            if isinstance(self.prev_model, MultiTaskModule):
                # output from previous output heads.
                with torch.no_grad():
                    y_prev = avalanche_forward(self.prev_model, x, fc, None)
                y_prev = {k: v for k, v in y_prev.items()}
                # in a multitask scenario we need to compute the output
                # from all the heads, so we need to call forward again.
                # TODO: can we avoid this?
                y_curr = avalanche_forward(curr_model, x, fc, None)
                y_curr = {k: v for k, v in y_curr.items()}
            else:  # no task labels. Single task LwF
                with torch.no_grad():
                    y_prev = {0: self.prev_model(x, fc)}
                y_curr = {0: out}
    ...
    def __call__(self, mb_x, mb_fc, mb_pred, model):
        """
        Add distillation loss
        """
        alpha = (
        ...
        return alpha * self._lwf_penalty(mb_pred, mb_x, mb_fc, model)
...
__all__ = [
    ...
    "LearningWithoutForgettingRAADL",
    "ACECriterion",
    "AMLCriterion",
]
