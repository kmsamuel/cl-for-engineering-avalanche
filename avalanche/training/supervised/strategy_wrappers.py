...
#         # # For shapenet, drivaernet
#         # epoch_in_cycle = epoch % 200 # 200 for drivaernet (++) PC
#         epoch_in_cycle = epoch % 150 #  for drivaernet parametric
        
#         # if epoch_in_cycle >= 150: # for drivaernet(++) pointclouds
#         #     current_lr = initial_lr * (0.1 ** 3)
#         # elif epoch_in_cycle >= 100: # DN(++) pointclouds
...
#         if self.scheduler is None: # don't do with SHIPD-Par
...
#         self.current_epoch += 1     
    ...
    #     # effective_epoch = self.current_epoch % 100 # for meta materials
    #     effective_epoch = self.current_epoch
    #     self.adjust_learning_rate(self.optimizer, effective_epoch) # for now, comment out if using anything but PC methods

    #     current_lr = self.optimizer.param_groups[0]['lr']
    #     print('**opt LR**', current_lr)

    #     # current_lr_sc = self.scheduler.optimizer.param_groups[0]['lr']
    #     # print('**sched LR**', current_lr_sc)
...
class Naive(ConfigurableLRMixin, SupervisedTemplate):
    """
    Naive finetuning with configurable learning rate scheduling.
    ...
    Now with configurable LR scheduling instead of hardcoded adjustments.
    """
    ...
    def __init__(
        ...
        **base_kwargs
        ...
        :param eval_every: the frequency of the calls to `eval` inside the
            ...
            `eval` is called every `eval_every` epochs and at the end of the
            learning experience.
        :param lr_config: Learning rate configuration dictionary.
        :param base_kwargs: any additional
            :class:`~avalanche.training.BaseTemplate` constructor arguments.
        """
    ...
    def training_epoch(self, **kwargs):
        ...
        # Apply the configurable LR scheduling
...
class GenerativeStrategy(SupervisedTemplate):
    """Naive finetuning.
    ...
    def training_epoch(self, **kwargs):
        """Training epoch."""
        # Adjust the learning rate at the beginning of the epoch
        effective_epoch = self.current_epoch % 100
        self.adjust_learning_rate(self.optimizer, effective_epoch)
