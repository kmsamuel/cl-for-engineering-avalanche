...
class EWCPlugin(SupervisedPlugin):
    ...
    def compute_importances(
        ...
        # for i, (data, label, target) in enumerate(dataloader):
            # get only input, target and task_id from the batch
            x, y, task_labels = batch[0], batch[1], batch[-1]
            # x, y = x.to(device), y.to(device)   # normal implementation
            ...
            if isinstance(x, dict):
                ...
                # Original behavior for when x is a single tensor
                x = x.to(device)

            # For y, assuming it's a single tensor
            y = y.to(device)
                                 
            optimizer.zero_grad()
            out = avalanche_forward_base(model, x, y)
...
class EWCPluginRAADL(SupervisedPlugin):
    """
    Elastic Weight Consolidation (EWC) plugin.
    EWC computes importance of each weight at the end of training on current
    experience. During training on each minibatch, the loss is augmented
    ...
    def compute_importances(
        ...
        # for i, batch in enumerate(dataloader):
        for i, (data, fltcond, label) in enumerate(dataloader):
            # get only input, target and task_id from the batch
            # x, y, task_labels = batch[0], batch[1], batch[-1]
            # x, y = x.to(device), y.to(device)
            data, fltcond, label = data.to(device), fltcond.to(device), label.to(device)
            
            optimizer.zero_grad()
            out = avalanche_forward(model, data, fltcond, label)
