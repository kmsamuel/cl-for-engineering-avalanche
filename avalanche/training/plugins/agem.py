...
class AGEMPlugin(SupervisedPlugin):
    ...
    def before_training_iteration(self, strategy, **kwargs):
        ...
        if len(self.buffers) > 0:
            ...
            out = avalanche_forward_base(strategy.model, xref, tid)
...
class AGEMPluginRAADL(SupervisedPlugin):
    """Average Gradient Episodic Memory Plugin RAADL.
    ...
    def before_training_iteration(self, strategy, **kwargs):
        ...
        if len(self.buffers) > 0:
            ...
            strategy.optimizer.zero_grad()
            mb = self.sample_from_memory()
            xref, fcref, yref, tid = mb[0], mb[1], mb[2], mb[-1]
            xref, fcref, yref = xref.to(strategy.device), fcref.to(strategy.device), yref.to(strategy.device)

            out = avalanche_forward(strategy.model, xref, fcref, tid)
            loss = strategy._criterion(out, yref)
            loss.backward()
            # gradient can be None for some head on multi-headed models
            reference_gradients_list = [
