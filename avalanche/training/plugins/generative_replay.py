...
class GenerativeReplayPlugin(SupervisedPlugin):
    ...
    def before_training_iteration(self, strategy, **kwargs):
        ...
        ## Non RAADL case:
        # replay = self.old_generator.generate(number_replays_to_generate)[0].to(
        #     strategy.device
        # )        
        voxel_replay = self.old_generator.generate(number_replays_to_generate)[0].to(
            strategy.device
        )
        fltparam_replay = self.old_generator.generate(number_replays_to_generate)[1].to(
            strategy.device
        )
        ...
        strategy.mbatch[0] = torch.cat([strategy.mbatch[0], voxel_replay], dim=0)
        strategy.mbatch[1] = torch.cat([strategy.mbatch[1], fltparam_replay], dim=0)
        
        print(voxel_replay)
        print(fltparam_replay)
        
        ## edit so that it returns mbatch 0 and 1 for voxels and flight params
        ...
        if not self.model_is_generator:
            with torch.no_grad():
                ...
                print('******************replay output*******************')
                replay_output = self.old_model(voxel_replay, fltparam_replay).argmax(dim=-1).float()

        else:
            # Mock labels:
            replay_output = torch.zeros(voxel_replay.shape[0])
        
        ## Non RAADL case
        # strategy.mbatch[1] = torch.cat(
        #     [strategy.mbatch[1], replay_output.to(strategy.device)], dim=0        

        strategy.mbatch[2] = torch.cat(
            [strategy.mbatch[2], replay_output.unsqueeze(1).to(strategy.device)], dim=0
        )
        ...
        #     [
        #         strategy.mbatch[-1],
        #         torch.ones(voxel_replay.shape[0]).to(strategy.device)
        #         * strategy.mbatch[-1][0],
        #     ],
        #     dim=0,
        # )
