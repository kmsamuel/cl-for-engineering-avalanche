...
class LRSchedulerPlugin(SupervisedPlugin):
    ...
    def after_training_exp(self, strategy: "SupervisedTemplate", **kwargs):
        ...
        if isinstance(self.scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
            # For ReduceLROnPlateau, we don't reset the lr in the same way
            if self.reset_lr:
                # Optionally, reset learning rates to base learning rates
                base_lrs = self.scheduler.optimizer.param_groups[0]["initial_lr"]
                for group in param_groups:
                    group["lr"] = base_lrs

            # For ReduceLROnPlateau, there is no `last_epoch` or reset method to call
            # We only step based on validation loss, so we don't need to reset it
    ...
    def _patch_lr_on_plateau(scheduler):
        ...
        if isinstance(scheduler, ReduceLROnPlateau):
            # ReduceLROnPlateau does not have `optimizer` or `base_lrs`
    ...
    def _step_scheduler(self, strategy: "SupervisedTemplate", **kwargs):
        if strategy.is_training:
            ...
            if self.metric == "train_loss" or self.metric != "val_loss":
                ...
                # Add this line to ensure the scheduler steps on val_loss during training if applicable
                # print("Validation Loss is training:", self.rolling_metric.result())
                self.scheduler.step(metrics=self.rolling_metric.result())
        else:
            # Validating
            if self._check_first_epoch_or_experience(strategy):
            ...
            if self.metric == "val_loss":
                # Add this line to ensure the scheduler steps on val_loss during training if applicable
                # print("Validation Loss:", self.rolling_metric.result())
                self.scheduler.step(metrics=self.rolling_metric.result())
