...
def avalanche_forward(model, voxels, flt_conds, task_labels):
    if is_multi_task_module(model):
        return model(voxels, flt_conds, task_labels)
    else:  # no task labels
        return model(voxels, flt_conds)
    
def avalanche_forward_base(model, x, task_labels):
    if is_multi_task_module(model):
