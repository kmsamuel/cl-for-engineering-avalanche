...
class ConfusionMatrix(Metric[Tensor]):
    ...
    def update(self, true_y: Tensor, predicted_y: Tensor) -> None:
        ...
        if len(predicted_y.shape) > 1:
            ...
            predicted_y_temp = torch.max(predicted_y, 1)[1]
            
            ## NEW REGRESSION CODE (for RAADL)
            # for ind, el in enumerate(predicted_y):
            #     if el <= 0.005:
            #         predicted_y_temp[ind] = 0
            #     elif 0.005 < el <= 0.01:
            #         predicted_y_temp[ind] = 1
            #     elif 0.01 < el <= 0.025:
            #         predicted_y_temp[ind] = 2
            #     elif el > 0.025:
            #         predicted_y_temp[ind] = 3
            # predicted_y = predicted_y_temp
            
            # REG CODE (for AGEDB-DIR)
            for ind, el in enumerate(predicted_y):
                if el <= 33:
                    predicted_y_temp[ind] = 0
                elif 33 < el <= 43:
        ...
        if len(true_y.shape) > 1:
            ...
            true_y_temp = torch.max(true_y, 1)[1]
            
            ## NEW REGRESSION CODE (for RAADL)
            # for ind, el in enumerate(true_y):
            #     if el <= 0.005:
            #         true_y_temp[ind] = 0
            #     elif 0.005 < el <= 0.01:
            #         true_y_temp[ind] = 1
        ...
        # print('true y 2', true_y)
        # print('pred y 2', predicted_y)
