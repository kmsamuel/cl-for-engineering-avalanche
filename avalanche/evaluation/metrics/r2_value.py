...
class R2_value(Metric[Tensor]):
    ...
    def result(self) -> float:
        ...
        if len(true_y[0]) > 1:
            r2_per_dim = [r2_score(true_y[:, i], predicted_y[:, i]) for i in range(true_y.shape[1])]
            
            return r2_score(true_y, predicted_y), r2_per_dim
...
class StreamR2(PluginMetric[Tensor]):
    ...
    def _package_result(self, strategy: "SupervisedTemplate") -> MetricResult:
        ...
        if self.save_image:
            ...
            fig, (ax, ax_pred) = plt.subplots(1, 2, figsize=(18, 6))
...
class MultiDimStreamR2(PluginMetric[Tensor]):
    """
    The Stream R2 value metric.
    This plugin metric only works on the eval phase
    ...
    def _package_result(self, strategy: "SupervisedTemplate") -> MetricResult:
        ...
        if self.save_image:
            # Create a 6x2 grid for the subplots (3 rows, 2 columns)
            ...
            for i in range(true_y.shape[1]):
                ax_scatter = axes[2 * i]  # Scatter plot on the left (odd columns)
                ax_kde = axes[2 * i + 1]  # KDE plot on the right (even columns)

                # Scatter plot
                ax_scatter.scatter(true_y[:, i], predicted_y[:, i], alpha=0.5)
                model = LinearRegression()
                true_y_reshaped = true_y[:, i].reshape(-1, 1)
                pred_y_reshaped = predicted_y[:, i].reshape(-1, 1)
                model.fit(true_y_reshaped, pred_y_reshaped)
                predicted_fit = model.predict(true_y_reshaped)
                slope = model.coef_[0].item()
                ax_scatter.plot(true_y[:, i], predicted_fit, color='red', linewidth=2, label=f'Slope: {slope:.2f}')
                
                # R² score
                r2_value = r2_score(true_y[:, i], predicted_y[:, i])
                ax_scatter.text(0.05, 0.95, f'R² = {r2_value:.2f}', transform=ax_scatter.transAxes, fontsize=14,
                                verticalalignment='top', bbox=dict(boxstyle='round', facecolor='white', alpha=0.5))

                ax_scatter.set_xlabel('True Values')
                ax_scatter.set_ylabel('Predicted Values')
                ax_scatter.set_title(f'Stream Regression Plot: Predicted Value {i + 1}')
                ax_scatter.legend()

                # Set x and y axis limits based on true values range
                max_val = true_y[:, i].max()
                min_val = true_y[:, i].min()
                ax_scatter.set_xlim([min_val, max_val])
                ax_scatter.set_ylim([min_val, max_val])

                # KDE plot
                scaled_true_y = np.ravel(true_y[:, i])
                scaled_predicted_y = np.ravel(predicted_y[:, i])

                data_true = pd.DataFrame({'Value': scaled_true_y, 'Label Type': 'True'})
                data_pred = pd.DataFrame({'Value': scaled_predicted_y, 'Label Type': 'Predicted'})
                data = pd.concat([data_true, data_pred])

                sns.kdeplot(data=data, x='Value', hue='Label Type', ax=ax_kde, fill=True, common_norm=False, common_grid=True, bw_adjust=0.5)
                ax_kde.set_title(f'Predicted Value {i + 1} Distribution')
                ax_kde.set_xlabel('Scaled Value')
                ax_kde.set_ylabel('Density')

                # Set KDE axis limits based on the true values range
                ax_kde.set_xlim([min_val, max_val])
...
def r2_metrics(
    ...
    if stream and not multi_dims:
        metrics.append(
            StreamR2(save_image=save_image)
        )
    if stream and multi_dims:
        metrics.append(
            MultiDimStreamR2(save_image=save_image)
        )
